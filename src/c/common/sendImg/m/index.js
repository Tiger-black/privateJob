...
$.extend(Index.prototype, Event, {
    ...
    events:function (){
        ...
        $(this.el).on('touchstart','.tips',function(e){
            self.clientX = e.changedTouches[0].clientX;
            $('.empty').addClass('emptycheck')
            self.flog = true;
        });
        $(this.el).on('touchend','.tips',function(e){
            self.clientX = 0;
            self.flog = false;
            $('.empty').removeClass('emptycheck');
            // if((new Date() - self.dateShiJian)>self.data.ttl){
            //     self.refreshImgData();
            //     new Toast('拼图失败，请重试');
            // }else{
        ...
        $(this.el).on('touchmove','.tips',function(e){
            e.preventDefault();
            self.moveX = e.changedTouches[0].clientX - self.clientX;
