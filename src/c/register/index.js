...
$.extend(Register.prototype, Event, {
    ...
    events: function () {
        ...
        this.el.find('.identify').on('click', function (event) {
            ...
            self.trigger('sendCodeClick', self.value);
    ...
    submitRegister: function () {
        ...
        if (!this.flag) {
            ...
            if (checkItem) {
                ...
                if (self.data.isMangoPage) {
                    self.data.mangoData.uuid = PasswordHash(this.userData.unionId.substring(5));
                    _data.mangoData = JSON.stringify(self.data.mangoData);
                }

                API.call({
                    url: self.data.isMangoPage ? API.registerMango : API.register, 
                    data: _data
                }, function (res) {
                    self.flag = false;
                    if (res.code == 0) {
    ...
    telFocus: function () {
        ...
        this.phoneInput.on('keyup', function () {
            ...
            if(self.data.isIdentifyDisable){
                if (length == 11 && Verifica.isTel(_val)) {
